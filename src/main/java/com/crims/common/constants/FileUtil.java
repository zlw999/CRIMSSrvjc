package com.crims.common.constants;
...
public class FileUtil {
    ...
    public static List<TreeNode> getTemPlates(String path, String fatherName) {
        ...
        File[] files = file.listFiles();
        TreeNode node = null;
        for (File file1 : files) {   //循环目录下的文件列表
            node = new TreeNode();
            if(StringUtils.isNotBlank(fatherName)){  //如果不为空 证明有上级 上级+//+本级
                node.setName(fatherName.concat("\\").concat(file1.getName()));      //设置名字
            }else {
                node.setName(file1.getName());
            }
            if (file1.isDirectory()){
                node.setType("1");    //如果是文件夹 set1
                String concat = path.concat("/").concat(file1.getName());
                List<TreeNode> temPlates = getTemPlates(concat,node.getName());  //所有的子级
                for (TreeNode node1:temPlates){
                    node1.setFatherName(node.getName());
                }
                node.setChildren(temPlates);
            }else{
                node.setType("2");
            }
    ...
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        InputStream resourceAsStream = FileUtil.class.getResourceAsStream("/");
        String path1 = FileUtil.class.getProtectionDomain().getCodeSource().getLocation().getPath();
        System.out.println(path1);
        System.out.println(conTextPath);
        String isoToUtf_8 = getIsoToUtf_8("%e4%b8%81%e6%98%8a%e5%a4%a9");
        String decode = URLDecoder.decode("/C:/Users/%e4%b8%81%e6%98%8a%e5%a4%a9/Desktop/ccspadmin-hd-master(19.1224)/target/classes/", "utf-8");
        String concat = conTextPath.concat("/src/main/resources/templates");
        File file = new File(concat);
        System.out.println(concat);
        File file2 = new File(property);
        String s = "C:/Users/丁昊天/Desktop/ccspadmin-hd-master(19.1224)/src/main/resources/templates";
        File file1 = new File(s);
//        List<TreeNode> temPlates = FileUtil.getTemPlates(s);
//        System.out.println(temPlates);
        System.out.println(file1.isDirectory());
        File[] files = file1.listFiles();
        System.out.println(files);
        HashMap<String, Object> filesName = FileUtil.getFilesName(s, "");
